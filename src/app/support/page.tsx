...
import { getAdminSupportTickets } from "@/lib/admin-api";
import type { SupportTicketStatus } from "@/lib/admin-types";
import { parseCursor, parseLimit, parseSupportTicketStatus } from "@/lib/format";

const DEFAULT_LIMIT = 20;

const STATUS_FILTERS: Array<{ label: string; status?: SupportTicketStatus }> = [
  { label: "All" },
...
export default async function SupportPage({
  ...
  const status = parseSupportTicketStatus(params.status);
  const result = await getAdminSupportTickets({ limit, cursor, status });

  return (
    <section>
      <PageHeader
        title="Support"
        description="Read-only support tickets from GET /v1/admin/support/tickets. Search filters the current page."
        mock={result.mock}
    ...
    <nav aria-label="Filter by status" className="mb-3 flex flex-wrap gap-2">
      {STATUS_FILTERS.map((filter) => {
        ...
        return (
          <Link
            key={filter.label}
            href={supportPath({ limit, status: filter.status })}
...
function Pagination({
  ...
  const firstHref = supportPath({ limit, status });
  const nextHref = nextCursor
    ? supportPath({ limit, status, cursor: nextCursor, keepLimit: true })
    : null;
